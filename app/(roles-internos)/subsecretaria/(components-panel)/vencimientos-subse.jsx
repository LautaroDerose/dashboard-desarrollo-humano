...
export default function TablaSubseVencimientos({ proximosVencimientos = [] }) {
  // Inicializa el estado local con los vencimientos recibidos
    ...
    try {
      ...
      setVencimientos((prevVencimientos) =>
        prevVencimientos.map((item) =>
          item.id === id
            ? { ...item, subsidy_stage: { ...item.subsidy_stage, note_doc: { is_confirm: !currentStatus } } }
            : item
        )
      );
        ...
        <Table>
          ...
          <TableBody>
            {vencimientos.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7}>No hay vencimientos disponibles.</TableCell>
              </TableRow>
            ) : (
