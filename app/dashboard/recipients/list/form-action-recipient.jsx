...
export default function FormActionRecipient({ recipient }) {
  ...
  const handleSocialConditionChange = (condition) => {
    if (selectedSocialConditions.includes(condition)) {
      setSelectedSocialConditions(selectedSocialConditions.filter((item) => item !== condition));
    } else {
      setSelectedSocialConditions([...selectedSocialConditions, condition]);
    }
  };
    ...
    <div>
      <form action={functionAction} className="flex flex-col gap-y-4">
        ...
        <select name="sex" className="w-full px-2 py-1 rounded-sm">
          <option value="">{recipient?.id ? recipient?.sex : "Indique el sexo correspondiente"}</option>
        ...
        >
          <option value="">{recipient?.id ? recipient?.contact_info.locality.name : "Seleccione una localidad"}</option>
        ...
        >
          <option value="">{recipient?.id ? recipient?.contact_info.street.name : "Seleccione una calle"}</option>
          {streets.map((street) => (
        ...
        {/* Simplified multi-select checkboxes */}
        <div className="w-full px-2 py-1 rounded-sm border">
          <p>Seleccione una o varias condiciones:</p>
          {socialConditions.map((condition) => (
            <label key={condition.id} className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={selectedSocialConditions.includes(condition.name)}
                onChange={() => handleSocialConditionChange(condition.name)}
              />
        ...
        {/* Hidden input to store selected social conditions */}
        <input
          type="hidden"
