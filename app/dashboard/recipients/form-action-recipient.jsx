 ...
 {/* <input type="hidden" name="birth_date" value={birthDate ? format(birthDate, 'yyyy-MM-dd') : ""} />
         <Popover>
          <PopoverTrigger asChild>
            <Button
              variant={"outline"}
              className={cn("w-full h-10 justify-start text-left font-normal", !birthDate && "text-muted-foreground")}
            >
              <FiCalendar className="mr-2 h-4 w-4" />
              {birthDate ?  new Date(new Date(recipient?.birth_date).getTime() + 86400000).toLocaleDateString("es-ES", {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                    })  : birthDate || <span>Seleccione una fecha</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent align="start" className="w-auto p-0">
            <Calendar
